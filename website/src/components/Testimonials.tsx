      ...
      <div className="relative max-w-7xl mx-auto px-6" ref={ref}>
        ...
        >
          <div className="inline-flex items-center gap-2 px-4 py-2 rounded-full glass mb-6">
            <span className="text-teal-accent dark:text-teal-accent text-teal-600 text-sm font-medium">Testimonials</span>
          </div>
        ...
        <div className="grid md:grid-cols-2 lg:grid-cols-3 gap-6">
          {testimonials.map((testimonial, index) => (
            <motion.div
              ...
              className="glass rounded-2xl p-8 hover:bg-white/5 transition-colors group"
            >
              {/* Stars */}
              <div className="flex gap-1 mb-6">
                {[...Array(testimonial.rating)].map((_, i) => (
                  <svg key={i} className="w-5 h-5 text-soft-amber" fill="currentColor" viewBox="0 0 20 20">
              ...
              {/* Quote */}
              <p className="dark:text-white/70 text-slate-900 leading-relaxed mb-8 dark:group-hover:text-white/90 group-hover:text-slate-950 transition-colors">
                "{testimonial.quote}"
              </p>

              {/* Author */}
              <div className="flex items-center gap-4">
                <div className="w-12 h-12 rounded-full bg-gradient-to-br from-teal-accent to-teal-accent/50 dark:from-teal-accent dark:to-teal-accent/50 from-teal-500 to-teal-400 flex items-center justify-center text-midnight dark:text-midnight text-white font-bold">
                  {testimonial.avatar}
                </div>
                <div>
                  <div className="font-semibold dark:text-white text-slate-800">{testimonial.author}</div>
                  <div className="text-sm dark:text-white/50 text-slate-800">{testimonial.role}</div>
                </div>
              </div>
            </motion.div>
          ))}
